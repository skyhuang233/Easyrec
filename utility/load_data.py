...
def make_pretrain_embedder_supervised_data_module(tokenizer:PreTrainedTokenizer, data_args):
    ...
    兼容没有 trn_dataset/val_dataset/max_seq_length 的情况，从路径自动推断。
    """
    from pathlib import Path

    def _infer_dataset(da):
        # 从已提供的路径中推断数据集名（取 data/ 之后的下一级目录）
        for name in ("train_data", "val_data", "user_profiles", "item_profiles"):
            p = getattr(da, name, None)
            if p:
                parts = Path(p).parts
                if "data" in parts:
                    i = parts.index("data")
                    if i + 1 < len(parts):
                        return parts[i + 1]
        # 再不行就看 data_path 目录下是否只有一个子目录
        dp = getattr(da, "data_path", "data")
        try:
    ...
    data_path = getattr(data_args, "data_path", "data")
    dataset = getattr(data_args, "trn_dataset", None) or _infer_dataset(data_args)
    val_dataset = getattr(data_args, "val_dataset", None) or dataset

    # max_seq_length 兜底：优先 data_args.max_seq_length，否则用 tokenizer.model_max_length（上限 512）
    max_seq_length = getattr(data_args, "max_seq_length", None)
    if not max_seq_length:
        ...
        try:
            ...
            m = 512
        max_seq_length = m if 0 < m < 10000 else 512

    used_diverse = int(getattr(data_args, "used_diverse_profile_num", 0) or 0)
    total_diverse = int(getattr(data_args, "total_diverse_profile_num", 0) or 0)
    add_raw_meta = bool(getattr(data_args, "add_item_raw_meta", False))
    ...
    train_dataset = LazyPretrainEmbedderDataset(
        ...
        used_diverse_profile_num=used_diverse,
        add_item_raw_meta=add_raw_meta,
    )

    eval_dataset = {}
    for _ds in str(val_dataset).split("-"):
        eval_dataset[_ds] = PretrainEmbedderAllRankTestDataset(
            data_path=data_path, dataset_name=_ds
        )
        for i in range(total_diverse):
            eval_dataset[f"{_ds}_diverse_{i}"] = PretrainEmbedderAllRankTestDataset(
                data_path=data_path, dataset_name=_ds, diverse_profile_no=i
            )

    data_collator = DataCollatorForPretrainEmbedderDataset(
        mlm_probability=getattr(data_args, "mlm_probability", 0.15), tokenizer=tokenizer
    )
    return dict(
